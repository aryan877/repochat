...
export default function ChatPage() {
  ...
  const filteredThreads = useMemo(() => {
    ...
    const list = threads.items.filter((t: { id: string }) => {
    ...
    return list.filter((t: { name?: string }) =>
      (t.name || "New conversation").toLowerCase().includes(query)
    );
  }, [threads?.items, threadSearch, deletedThreadIds]);
          ...
          <div className="space-y-0.5">
            {threadListResult.isPending ? (
              ...
              filteredThreads.map((t) => (
                ...
                >
                  <MessageIcon />
                  <span className="truncate flex-1">{t.name || "New conversation"}</span>
                  <button
